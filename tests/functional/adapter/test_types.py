...
from dbt.tests.adapter.utils.data_types.test_type_timestamp import BaseTypeTimestamp, seeds__expected_csv, seeds__expected_yml
...
def run_around_tests(unique_schema, adapter):
    ...
    with get_connection(adapter):
        relations = adapter.list_relations(adapter.config.credentials.database, unique_schema)
        expected_relations = [rel for rel in relations if rel.identifier.lower() == "expected"]
...
@pytest.mark.skip(reason="Netezza requires `varchar` length to be specified and `text` type results in CLOB error.")
